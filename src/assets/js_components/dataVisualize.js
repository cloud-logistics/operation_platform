function dataV(canvas,ctx, conf) {
    if (!ctx) {
...
dataV.prototype = {
    ...
    render: function () {
        ...
        this.drawBottomLine();
        this.drawBottomText();
        if(Number(this.conf.value) > this.conf.maxValue){
            var value = this.conf.maxValue;
        }else if(Number(this.conf.value) < this.conf.minValue){
            var value = this.conf.minValue;
        }else{
            var value = Number(this.conf.value);
        }
        this.drawValue(this.conf.outerCircleStartAngle + angleRange / valueRange * (value - this.conf.minValue))
    }
};