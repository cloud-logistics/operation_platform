...
angular.module('ngPagination',[]).directive('tmPagination', [function () {
    return {
        ...
        '<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="lastPage()"><span >尾页</span></li>' +
        ...
        '<span>页</span>'+
        '<span >每页</span>'+
        '<select ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions "></select>' +
        '条/ <span >共</span><strong>' + " " + '{{ conf.totalItems }}' + " "+'</strong><span >条</span>' +
        '</div>' +
